...
function party_info() {
  $.ajax({
    url: 'http://127.0.0.1:6969/party_info',
    type: 'GET',
    dataType: 'json',
    success: function(data) {
      ...
      for (var i = 0; i < party.length; i++) {
        var pokemon = party[i];
        var index = i + 1; // Adjust the index to match the numbering convention (e.g., party1, party2, etc.)

        // Update party name and level
        $('#party' + index + '_name').text(pokemon.name);
        $('#party' + index + '_level').text(pokemon.level);

        // Update party nature, types, hidden power, held item, and sprite
        if (pokemon.shiny) {
          $('#party' + index + '_sprite').attr('src', 'sprites/pokemon/shiny/' + pokemon.name + '.png');
        } else {
          $('#party' + index + '_sprite').attr('src', 'sprites/pokemon/' + pokemon.name + '.png');
        }
        $('#party' + index + '_nature').text(pokemon.nature);
        $('#party' + index + '_type').html('<img src="sprites/types/' + pokemon.type_1 + '.png">');
        $('#party' + index + '_type2').html('<img src="sprites/types/' + pokemon.type_2 + '.png">');
        $('#party' + index + '_hidden_power_type').html('<img src="sprites/types/' + pokemon.hiddenPowerType + '.png">');
        $('#party' + index + '_item').text(pokemon.itemName);
        $('#party' + index + '_item_image').attr('src', 'sprites/items/' + pokemon.itemName + '.png');

				// Update moves and move PP
		for (var j = 0; j < pokemon.enrichedMoves.length; j++) {
		  var move = pokemon.enrichedMoves[j];
		  var moveIndex = j + 1;
		  var movePP = pokemon['move_' + moveIndex + '_pp'];

		  $('#party' + index + '_move_' + moveIndex).text(move.name);
		  $('#party' + index + '_move_' + moveIndex + '_pp').text(movePP + '/' + move.pp);
		}

		// Update maxPP
		for (var j = 0; j < 4; j++) {
		  var moveIndex = j + 1;
		  var maxMovePP = pokemon.enrichedMoves[j].pp;

		  $('#party' + index + '_move_' + moveIndex + '_maxPP').text(maxMovePP);
		}

        // Update HP bar
        updateHPBar(pokemon, index);
      }
    },
    error: function(xhr, status, error) {
      console.log("Error retrieving party information.");
    }
  });
}

function updateHPBar(pokemon, index) {
  var hpBar = $('#party' + index + '_health-bar-fill');
  
  // Calculate the HP percentage
  var hpPercentage = (pokemon.hp / pokemon.maxHP) * 100;
  
  // Update the width of the health bar fill based on the HP percentage
...
function getTypeImages(types) {
  ...
  if (Array.isArray(types)) {
    for (var i = 0; i < types.length; i++) {
      typeImages += `<img src="${getTypeImageURL(types[i])}" alt="${types[i]}">`;
    }
  } else {
    typeImages = `<img src="${getTypeImageURL(types)}" alt="${types}">`;
  }
  return typeImages;
}

function getTypeImageURL(type) {
  // Map type names to corresponding image URLs
  var typeImageMap = {
    'Bug': 'bug.png',
    'Dark': 'dark.png',
    'Dragon': 'dragon.png',
  ...
  if (typeImageMap.hasOwnProperty(type)) {
    return 'interface/sprites/types/' + typeImageMap[type];
  }

  // If the type is not found, return a default image URL
  return 'interface/sprites/types/default.png';
}

// Call the party_info function to retrieve and update the data
party_info();
